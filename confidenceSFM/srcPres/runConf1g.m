...
if (demoOn)
    ...
    Screen(window,'DrawText','Press 7 if you do not want to bet',200,450);
    ...
    Screen(window,'WaitBlanking');
    Screen(window,'DrawText','Press 7: ',200,325);
    waitFor('7');
    Screen(window,'DrawText','7 (left tilt, no bet)');
    Screen(window,'WaitBlanking');
    Screen(window,'DrawText','Press 8: ',200,350);
    waitFor('8');
    Screen(window,'DrawText','8 (right tilt, no bet)');
     
    Screen(window,'WaitBlanking');
    Screen(window,'DrawText','As soon as you are ready, press any key, to start eyetracker calibration.',100,600);
    waitForKey;
end
...
for trial = 1 : nTrials
    ...
    try
        ...
        Screen(window,'WaitBlanking');
        Screen(window,'DrawText',...
            '1 : \ ($2 bet)   2 : \ ($1 bet)       3: \ (50 Cents bet)          4: / (50 Cents bet)  5: / ($1 bet)      6: / ($2 bet)  ',25,400,0);
        
         Screen(window,'DrawText',...
            'I want to bet',450,350,0);
        Screen(window,'DrawText',...
            'I do not want to bet',450,500,0);
         Screen(window,'DrawText',...
            '7 : \        8 : / ',450,550,0);
        Screen(window,'DrawText',...
            '?',500,250,0);
