...
sf            = [2,3,4,8,9,10];               % cycles per patchsize
targetPhase   = pi/2;               %
...
% change here
if ((length(sf)~=6)|(mod(nTrials,length(sf)*4)))
...
target = cell(2,2,length(sf)); % up/down, left/right, ori
mask   = cell(2,1);
...
for ud = 1 : 2
    ...
    for lr = 1 : 2
        ...
        for sfC = 1 : length(sf)
            ...
            target{ud,lr,sfC} = Screen(window,'OpenOffScreenWindow',lum.gray,[0 0 patchSize patchSize]);
            ...
            Screen(target{ud,lr,sfC},'PutImage',Glin);
...
if (demoOn)
   ...
   % Screen(window,'DrawText','And can occur in the upper or lower hemifield.',100,450);
    ...
    Screen(window,'WaitBlanking');
    Screen(window,'DrawText','As soon as you are ready, press any key, to start the eye tracker calibration.',100,400);
    waitForKey;
end
...
for trial = 1 : nTrials
    ...
    try
        ...
        Screen(window,'WaitBlanking');
        Screen(window,'DrawText',...
            '1 : low (certain)   2 : low       3: low (guessing)          4: high (guessing)  5: high      6: high (certain)  ',100,400,0);
        Screen(window,'DrawText',...
            '?',500,350,0);
