...
% 
% SND('Play',signal,[rate]) plays a sound (on both Mac and Win). Unlike
% Matlab's SOUND command, SND returns immediately, while the sound is
% still playing. On the Mac, the queue can have up to 128 sounds, so
% calling SND('Play',signal) while a sound is already playing adds the new
% sound to the queue. On Win, the queue can hold only one sound, so SND
% waits until the currently playing sound ends, then starts the new sound
% and returns immediately, while it's playing. The "signal" and "rate"
% arguments are explained below.
% 
% rate=SND('DefaultRate') returns the default sampling rate in Hz, which
% is 22254.5454545454 Hz on both Mac and Win platforms. This default may
% change in the future, so please either specify a rate, or use this
% function to get the default rate.
%
% Mac: Before playing SND internally converts the matrix of double-precision 
...
%
% Win: SND accepts the sampleSize argument and passes it to the Matlab
% SOUND command.  SOUND (and therefore SND also) obeys the specified
% sampleSize value, either 8 or 16, only if it is supported by your computer
% hardware.  
%  
% SND('Open') opens the channel, which stays open until you call
% SND('Close'). SND('Play',...) automatically opens the channel if it
% isn't already open. (At present the Win version ignores the 'Open'
% command.)
% 
...
% specified, the sample "rate", on both Mac and Win, defaults to the Mac's
...
% 
% At present, the Mac channel's queue can hold up to 128 sounds (which
% could easily be increased--let us know), and the Win channel's queue can
% hold only one. If the queue is full when you call SND('Play',signal),
...
% 
% Mac: SND('Play',signal) takes some time to open the channel, if it isn't
...
% (There is no opening overhead in the Win version.) However, the
...
% ******
%	
% Win: SND plays your sound using Matlab's SOUND function. 
% 
...
% by SND('DefaultRate').
% 
% See also MakeBeep, READSND, and WRITESND.
...
% 4/13/02	dgp Enhanced both Mac and Win versions so that 'DefaultRate'
...
if strcmp(computer,'PCWIN')
	...
	if strcmp(command,'Play')
        ...
        err = 22254.5454545454;;
	elseif strcmp(command,'Wait')
		if nargin>1
			error('Wrong number of arguments: see SND.');
		end
		WaitSecs(endTime-GetSecs); % Wait until any ongoing sound is done.
		err=0;
	elseif strcmp(command,'IsPlaying')
		if nargin>1
			error('Wrong number of arguments: see SND.');
		end
		if endTime>GetSecs
	...
	elseif strcmp(command,'Quiet') | strcmp(command,'Close')
		if nargin>1
			error('Wrong number of arguments: see SND.');
		end
		...
		endTime=0;
		err=0;
	elseif strcmp(command,'DefaultRate')
		if nargin>1
			error('Wrong number of arguments: see SND.');
		end
		err=22254.5454545454; % default sampling rate in Hz.
	elseif strcmp(command,'Open')
		endTime=0;
	else
...
% this is my bug-fix, makes it wait, but works wet
if strcmp(command,'Play')
    SND('Wait');
    SND('Quiet');
    SND('Close');
