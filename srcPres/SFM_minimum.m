...
%     asdf
    ...
    %% verify screen refresh rate (this is done internally by Psychtoolbox)
    % t0_test = GetSecs;
    % if 0 % 13 Mar 11 by NT
    %     Screen(Exp.Cfg.win,'WaitBlanking',10);
    %     t1_test = GetSecs;
...
%             eyelink('message', 'MARKERID %i', SIG_SER_ON);
        ...
        for iFrame = 1 : SFM.nframe
            ...
            %             screen('copywindow',Top(iFrame),Exp.Cfg.win,[],imgRectTop)
            % screen('copywindow',Bot(iFrame),Exp.Cfg.win,[],imgRectBot)
            
            
            % COLLECT CONTINUOUS RESPONSES HERE
        ...
        % Close all previous textures
        Screen('Close', dot_textures)
