...
for su = 1 : length(subjects)
    ...
    while (1)
        ...
        if ~isempty(strfind(tline, 'UserEvent')) && ~isempty(regexp(tline, 'START_BK','once'))
            ...
            bk_idx = bk_idx + 1;
            messg = sprintf('Subject: %s, Run: %d, block: %d', subjects{su}, Exp.rnd_idxs(run_idx), bk_idx);
            disp(messg)
    ...
    save([data_dir subjects{su} '_eye_beh'], 'Exp','Run', 'blinks', 'saccades', 'fixations')
    ...
    idxs = ~isnan(samps(:,1));
    samps = samps(idxs, :); %#ok
    save([data_dir subjects{su} '_eye_beh'], 'samps', '-append');
